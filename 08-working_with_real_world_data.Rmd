...
```
iii. Add a new column into the data set. Just the convert the temperature from $^oC$(degrees Celcius) to $^oF$(degrees Farenheit)
iv. Write the file locally and check the time taken to write the file. **Hint**: Here is how you can achieve step iii abd iv
```
# Do the Calculations
...
iv. Reread the data set however control the threads. Set different thread value (e.g 1, 2, 3, 4 and 5) the compare the time taken for processing. 
