...
- Filter the data set to include restaurants in specific locations: "Koramangala", "Indiranagar", and "MG Road".
- Create a subset that includes only restaurants serving both "North Indian" and "Chinese" cuisine.
- Filter out restaurants that have no reviews or ratings.

2. **Sorting**

- Sort the data set by restaurant ratings in descending order.
- Sort the data set first by location and then by average cost for two people in ascending order.
- Sort the data set by the number of votes a restaurant received, with the highest votes appearing first.

3. **Data Cleaning**

- Identify and remove rows with missing or null values in any key columns (e.g., ratings, cost, cuisine type).
- Correct inconsistent data types: Ensure that numeric columns (such as "cost for two") are in the correct numeric format.
- Remove any duplicate restaurant entries based on the restaurant name and location.
- Ensure consistency in categorical data, such as correcting case sensitivity or spelling issues (e.g., "North Indian" vs. "north indian").
- Identify and handle any outliers in the "cost for two" column.
...
```{r ex="activity-5a", type="solution", include=FALSE}
# Load the dplyr library
library(dplyr)

# Read the data 
data_path = "data/car_sales.csv"
car_sales = read.csv(data_path)
...
```{r ex="activity-5b", type="sample-code", include=FALSE}
# Load the dplyr library
library(dplyr)
...
```{r ex="activity-5c", type="solution", include=FALSE}
# Load the dplyr library
library(dplyr)
...
```{r ex="activity-5d", type="solution", include=FALSE}
# Load the dplyr library
library(dplyr)
...
```{r ex="actvity-5e", type="solution", include=FALSE}
# Load the dplyr library
library(dplyr)
...
```{r ex="activity-5f", type="solution", include=FALSE}
# Load the dplyr library
library(dplyr)
